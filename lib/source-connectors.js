...
function markdownJobs(source, markdown) {
  ...
  for (const line of content.split('\n')) {
    ...
    jobs.push({externalId:`markdown-${crypto.createHash('sha1').update(url).digest('hex')}`,company,title,location:location||'Location not listed',url,description:'',salary:'',skills:[],postedAt:new Date().toISOString(),source:source.name,earlyCareer:source.earlyCareer===true});
  }
  return jobs;
}
