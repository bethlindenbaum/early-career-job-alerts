...
const COMPANY_ALIASES = { 'walt disney': ['disney'], 'jp morgan': ['jpmorgan', 'jpmorgan chase'], 'twitter': ['x'], 'warner bros': ['warner bros. discovery', 'warner bros discovery'] };
function companyMatches(target, actual) {
  const expected = words(target); const candidate = words(actual);
  return expected === candidate || (COMPANY_ALIASES[expected] || []).includes(candidate);
}
...
function match(job, preferences) {
  if (!(job.earlyCareer || EARLY.test(job.title)) || EXCLUDE.test(job.title) || requiresPriorExperience(job.description)) return null;
  ...
  if (!companies.some(company => companyMatches(company, job.company))) return null;
...
function requiresPriorExperience(description) {
  const text = String(description || '');
  const patterns = [/\b(?:[2-9]|1\d)\+?\s+years?\b[^.!?\n]{0,100}\bexperience\b/gi, /\bexperience\b[^.!?\n]{0,60}\b(?:[2-9]|1\d)\+?\s+years?\b/gi];
  for (const pattern of patterns) for (const result of text.matchAll(pattern)) {
    const context = text.slice(Math.max(0, result.index - 100), Math.min(text.length, result.index + result[0].length + 100));
    if (!/(?:preferred|nice to have|bonus|a plus|not required|optional|up to|0\s*[-–]\s*[2-9])/i.test(context)) return true;
  }
  return false;
}
...
module.exports = { match, skillsFrom, requiresPriorExperience };
